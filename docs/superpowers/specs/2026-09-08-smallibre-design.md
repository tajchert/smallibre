# Smallibre: stack and architecture proposal
...
Across all choices, reuse Calibre's difficult format handling under GPL. Do not make a new MOBI/KF8 writer a prerequisite for shipping Smallibre. Keep originals immutable, create derived output on demand, and make transfer recovery a first-class feature.
...
Build a new Smallibre database and application UI. Calibre's library cache implements broad search/sort behavior in Python; Smallibre's narrower schema can use indexed SQLite queries and paged results. Do not edit a live Calibre library. An eventual migration reads a consistent snapshot and imports through Smallibre's normal API.

Calibre already separates conversion from targeted polishing and already offloads work to subprocesses. These are useful designs to preserve. Its broader scope and runtime dependencies are the opportunity to reduce size, not evidence that all existing code needs replacement. Detailed evidence appears in the [source audit](../../architecture/calibre-source-audit.md).
...
crates/smallibre-storage/               SQLite migrations, queries, blob repository
crates/smallibre-platform/              worker launcher, devices, HTTP adapters
crates/smallibre-bindings/              UniFFI facade for native Mac option
