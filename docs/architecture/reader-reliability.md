...
- Sends back up the exact prepared snapshot, export those same bytes without overwriting existing files, and record the outcome. Interrupted writes can leave hidden `.smallibre-*` device staging files; they are not automatically removed because their state may be uncertain.
