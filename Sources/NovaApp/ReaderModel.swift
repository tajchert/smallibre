...
@MainActor @Observable final class ReaderModel {
    ...
    var localRoot: URL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appendingPathComponent("Calibre Nova")
    var helper: URL { Bundle.main.executableURL!.deletingLastPathComponent().appendingPathComponent("NovaReaderHelper") }
    ...
    func run(_ action: String, books selected: [ReaderBook], model: AppModel, metadata: BookMetadata? = nil, edit: Bool = false) {
        guard !busy, let folder, rootIdentity != nil else { return }
