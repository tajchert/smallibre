...
/// C1-only prose subset. This is not the production EPUB normalizer: reject unsupported content
/// rather than silently losing it. One fragment per chapter, flat EPUB 3 navigation, PNG/JPEG images.
struct AZW3PrototypeDocument {
    struct Chapter { let path: String; let title: String; let body: String; let targets: [String: Int] }
    ...
    init(_ input: Data) throws {
        guard input.count <= 16 * 1024 * 1024 else { throw BookError.unsupported("The AZW3 prototype accepts EPUBs up to 16 MB.") }
        let epub = try EPUBBook(data: input)
        guard epub.allowsTypography, !epub.archive.names.contains("META-INF/encryption.xml"),
              epub.chapters.count <= 64, Set(epub.chapters).count == epub.chapters.count else {
            throw BookError.unsupported("The AZW3 prototype requires unencrypted reflowable chapters without repeated spine items.")
        }
        ...
        for node in try package.nodes(forXPath: "//*[local-name()='manifest']/*[local-name()='item']") {
            ...
            guard ["application/xhtml+xml", "image/png", "image/jpeg"].contains(mime) else {
                throw BookError.unsupported("The AZW3 prototype cannot preserve \(mime). CSS, fonts and other media need the production normalizer.")
            }
        ...
        guard let navigationPath else { throw BookError.unsupported("The AZW3 prototype currently requires EPUB 3 navigation.") }
        var imageMap: [String: Int] = [:], images: [Resource] = []
        ...
        for (chapterNumber, path) in epub.chapters.enumerated() {
            ...
            guard headChildren.allSatisfy({ $0.localName == "title" || $0.localName == "meta" }) else {
            ...
            func serialize(_ node: XMLNode) throws {
                ...
                guard let element = node as? XMLElement, let name = element.localName, supported.contains(name),
                ...
                for attribute in (element.attributes ?? []).sorted(by: { ($0.name ?? "") < ($1.name ?? "") }) {
                    ...
                    guard ["id", "class", "title", "href", "src", "alt", "width", "height", "lang"].contains(key) else {
                ...
                guard result.utf8.count <= 8192 else { throw BookError.unsupported("Prototype chapters must fit one 8 KB fragment.") }
            }
            for attribute in body.attributes ?? [] {
                guard attribute.name == "id" else { throw BookError.unsupported("Unsupported prototype body attribute.") }
                if let id = attribute.stringValue { targets[id] = 0 }
            }
            for child in body.children ?? [] { try serialize(child) }
            guard !result.isEmpty, result.utf8.count <= 8192 else { throw BookError.unsupported("Empty or oversized prototype chapter.") }
            parsed.append(Chapter(path: path, title: title, body: result, targets: targets))
        }
        ...
        chapters = parsed.enumerated().map { index, chapter in
            Chapter(path: chapter.path, title: chapter.title, body: String(decoding: bodies[index], as: UTF8.self), targets: chapter.targets)
        }
        ...
        let nav = navs[0]
        guard try nav.nodes(forXPath: ".//*[local-name()='li']//*[local-name()='li']").isEmpty else {
        ...
        guard !anchors.isEmpty, anchors.count <= 64 else { throw BookError.invalid("Invalid prototype navigation size.") }
