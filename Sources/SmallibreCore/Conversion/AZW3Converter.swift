...
public enum AZW3Converter {
    ...
    public static func convert(_ input: Data) throws -> Result {
        ...
        let document = try AZW3PrototypeDocument(input, production: true)
        let output = try AZW3PrototypeWriter.convert(input, document: document, production: true)
        try validate(output)
        return Result(data: output, warnings: document.warnings + ["Native AZW3 conversion supports a bounded reflowable EPUB subset. Navigation is flattened. Kindle may render CSS differently; review the converted book before relying on it."])
    }
