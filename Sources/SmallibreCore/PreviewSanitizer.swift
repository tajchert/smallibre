...
public enum PreviewSanitizer {
    public static func allowsNavigation(_ url: URL) -> Bool {
        url.scheme == "smallibre-book" && url.host == "book" && ["html", "xhtml", "htm"].contains(url.pathExtension.lowercased())
    }
    public static func html(_ data: Data) throws -> Data {
        ...
        policy.addAttribute(XMLNode.attribute(withName: "content", stringValue: "default-src 'none'; img-src smallibre-book: data:; style-src smallibre-book: 'unsafe-inline'; font-src smallibre-book: data:; script-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'") as! XMLNode)
