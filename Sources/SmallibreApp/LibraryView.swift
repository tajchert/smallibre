...
struct LibraryView: View {
    ...
    private var sidebar: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 10) {
                Image(systemName: "books.vertical.fill").font(.system(size: 25, weight: .light)).foregroundStyle(SmallibreTheme.accent)
                VStack(alignment: .leading, spacing: 1) { Text("Smallibre").font(.system(size: 17, weight: .semibold, design: .serif)); Text("A home for your books").font(.system(size: 10)).foregroundStyle(.secondary) }
    ...
    private func navigationRow(_ title: String, icon: String, key: String, count: Int) -> some View {
        Button { model.filter = key } label: {
            HStack(spacing: 10) { Image(systemName: icon).frame(width: 18); Text(title); Spacer(); Text("\(count)").font(.system(size: 11, design: .rounded)).foregroundStyle(.secondary) }
                ...
                .foregroundStyle(model.filter == key ? SmallibreTheme.accent : .primary)
                .background(model.filter == key ? SmallibreTheme.accent.opacity(0.11) : .clear, in: .rect(cornerRadius: 7))
    ...
    private var shelf: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            footer
        }.frame(maxWidth: .infinity, maxHeight: .infinity).background(SmallibreTheme.canvas)
    }
    private var emptyLibrary: some View {
        VStack(spacing: 18) {
            Image(systemName: "book.closed").font(.system(size: 55, weight: .ultraLight)).foregroundStyle(SmallibreTheme.accent).padding(.bottom, 6)
    ...
    private var footer: some View {
        VStack(spacing: 0) {
            Divider().opacity(0.5)
            HStack(spacing: 8) {
                if model.operation != nil { ProgressView().controlSize(.mini) } else { Image(systemName: "checkmark.shield").foregroundStyle(SmallibreTheme.accent) }
