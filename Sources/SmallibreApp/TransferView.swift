...
struct TransferView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            HStack(spacing: 14) {
                Image(systemName: "externaldrive").font(.system(size: 32, weight: .light)).foregroundStyle(SmallibreTheme.accent)
            ...
            if compatible {
                Text("Choose the books folder on your connected reader. Smallibre will copy and verify the file, keeping any existing books.").font(.callout).foregroundStyle(.secondary)
                Button {
