...
@main struct ReaderHelper {
    static func main() async {
        ...
        do {
            ...
            case "scan": response = ReaderResponse(books: try await store.scan(cacheURL: request.destination == nil ? request.localRoot.appendingPathComponent("reader-cache.json") : nil), rootIdentity: store.rootIdentity)
            case "download":
