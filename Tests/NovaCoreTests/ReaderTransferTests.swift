...
final class ReaderTransferTests: XCTestCase, @unchecked Sendable {
    func testMountedReaderRoundTrip() async throws {
        ...
        guard let sourcePath = env["NOVA_TEST_READER_BOOK"], let folderPath = env["NOVA_TEST_READER_FOLDER"] else {
            throw XCTSkip("Set NOVA_TEST_READER_BOOK and NOVA_TEST_READER_FOLDER for an authorized hardware test")
        }
        ...
        book.metadata.title = "Nova transfer test " + UUID().uuidString
