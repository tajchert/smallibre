...
final class ReaderHardwareTests: XCTestCase, @unchecked Sendable {
    func testDisposableMetadataAndBackedUpDelete() async throws {
        ...
        guard let devicePath = env["NOVA_HARDWARE_DEVICE"], let sourcePath = env["NOVA_HARDWARE_SOURCE"], let localPath = env["NOVA_HARDWARE_LOCAL"] else { throw XCTSkip("Opt-in disposable hardware workflow") }
        ...
        metadata.title = "Nova Reader Test — delete check"
        let prepared = try MOBIMetadataEditor.prepare(original, metadata: metadata)
        let name = "Nova Reader Test \(UUID().uuidString).mobi"
        ...
        metadata.title = "Nova Reader Test — before update"
        ...
        metadata.title = "Nova Reader Test — ready to read"
