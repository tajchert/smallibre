...
final class LocalBookTests: XCTestCase, @unchecked Sendable {
    func testLocalEPUBRoundTrip() async throws {
        guard let path = ProcessInfo.processInfo.environment["NOVA_TEST_EPUB"] else {
            throw XCTSkip("Set NOVA_TEST_EPUB to test a local book")
        }
