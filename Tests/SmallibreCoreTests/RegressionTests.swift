...
final class RegressionTests: XCTestCase {
    ...
    func testTitleEditPreservesCreatorAttributesAndRefinements() throws {
        ...
        XCTAssertEqual(try document.nodes(forXPath: "//*[@id='author1']").first?.stringValue, "The Smallibre Studio")
...
extension RegressionTests {
    func testEarlierTypographyMarkerIsReplacedAfterRename() throws {
        let url = Bundle.module.url(forResource: "Small Hours", withExtension: "epub", subdirectory: "Fixtures")!
        let source = try EPUBBook(data: Data(contentsOf: url))
        let path = try XCTUnwrap(source.chapters.first)
        let chapter = String(data: try source.archive.data(named: path), encoding: .utf8)!.replacingOccurrences(of: "</head>", with: "<style id=\"nova-typography\">body { color: red; }</style></head>")
        let input = try EPUBBook(data: source.archive.writing(replacements: [path: Data(chapter.utf8)]))
        let output = try EPUBBook(data: EPUBEditor.prepare(input, metadata: input.metadata, typography: TypographySettings(enabled: true)))
        let document = try SafeXML.document(output.archive.data(named: path))
        XCTAssertEqual(try document.nodes(forXPath: "//*[@id='smallibre-typography']").count, 1)
        XCTAssertEqual(try document.nodes(forXPath: "//*[@id='nova-typography']").count, 0)
    }
}
